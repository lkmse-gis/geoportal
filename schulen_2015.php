...
//globale Varibalen
$layername_mapfile="Schulen_2015";
$titel="Schulen 2015";
$titel_legende="Schule";
$scriptname="schulen_2015.php";
$tabelle="geoportal_schulen_2015";
$schema="geoportal";
$get_themenname="schule";
$layerid="100000";
$leg_bild="apotheke.gif";
									...
									<tr>
												...
												<?php for($v=0;$v<$z;$v++)
													{ 
														...
														if(strlen($bildname) < 1 OR $schulen_2015[$v][oeffentlich] == 'nein')
															{
																echo "<td></td>";	
															} 
														else 
														...
														echo "
														<td align='center' height='30'><a href=\"$scriptname?$get_themenname=",$schulen_2015[$v][gid],"\">",$schulen_2015[$v][bezeichnung],"</a></td>",
														"<td align='center'>",$anschrift,"</td>",
														"<td align='center'>",$schulen_2015[$v][mail],"</td>",
														"<td align='center'>",$schulen_2015[$v][tel],"</td></tr>";
													}
												?>																																				
											</table>
										</td>
...
<?  }
 ...
 if ($themen_id > 0)
   {   
	  ...
	  $query="SELECT astext(wkb_geometry) as utm, astext(st_transform(wkb_geometry,2398)) as gk4283,astext(st_transform(wkb_geometry, 4326)) as geo,astext(st_transform(wkb_geometry, 31468)) as rd83, oid,gid,gml_id,fa_adressschluessel,bezeichnung,ortsteil,gkz,geoportal_anschrift,stichtag,schul_id,schultyp,klassifizierung_schulen,klassifizierung_regis,isced_level,schularten,tel,fax,mail,internet,profile,schulleiter,schultraeger,schueler,fristgemaess,vorzeitig,verspaetet,aussiedler,asylbewerber,fluechtlinge,maennlich,fristgemaess_m,vorzeitig_m,verspaetet_m,aussiedler_m,asylbewerber_m,fluechtlinge_m,weiblich,fristgemaess_w,vorzeitig_w,verspaetet_w,aussiedler_w,asylbewerber_w,fluechtlinge_w,adressschluessel,kreis_name,kreisschluessel,gem_schl,gemeinde_name,strasse_name,strasse_schluessel,hausnummer,nummer,zusatz,postleitzahl,alkis_konform,bild,oeffentlich,wkb_geometry FROM $schema.$tabelle WHERE gid='$themen_id'";
							...
							<td valign=top>
								...
								<td valign=top>											
									<table border=0 valign=top>
										...
										<tr>
											...
											<?												
												...
												if(strlen($bildname) < 1 OR $oeffentlich == 'nein')
													{
														echo "<td valign=center align=right rowspan=9 width=420 height=235><table border=0 cellpadding=50><tr><td class='rand'><font color=red><b>kein Bild verfügbar<br>oder nicht freigegeben</b></font></td></tr></table></td>";	
													} 
												else 
													{
										...
										<tr>
											<td bgcolor=<? echo $element_farbe ?>>Schüler:</td>
											<td bgcolor=<? echo $element_farbe ?>><b><? echo $r[schueler],' (M: ',$r[maennlich],' / W: ',$r[weiblich],')'  ;?></b></td>																									
										</tr>										
									</table>
							</td>									
