...
if ($gemeinde_id > 0)
   { 
	  ...
	  $query="SELECT box(a.the_geom) as box, area(a.the_geom) as area, st_astext(st_centroid(st_transform(a.the_geom,4326))) as geo, st_astext(st_centroid(a.the_geom)) as center, st_astext(st_centroid(a.the_geom)) as utm, st_perimeter(a.the_geom) as umfang, a.gemeinde as name, b.gid as vsitzid, a.einwohner as einw, a.buergermeister as bm, a.einw_km as einw_km, a.wappen as wappen, a.vorwahl as vorwahl, a.plz as plz,st_buffer(a.the_geom,-20) as gemeindeumring  from gemeinden as a, fd_amtssitze_msp as b WHERE gem_schl='$gemeinde_id' AND a.amt_id=CAST(b.amt_id as character varying)";
	   ...
	   while($r = $fetcharrayp($result))
		...
		<script type="text/javascript" language="Javascript">
			...
			function load() {
				...
				var sg_60_2 = new OpenLayers.Layer.WMS.Untiled("Zuständigkeit",
								 <? echo $map_msp_url;?>,
								 {layers: 'sg_60_2', transparent: true, format: 'image/png'},
								 {isBaseLayer: false}
				);
				...
				map.addLayers([orka_mv,dop,sg_60_2,msp_outline,amt_outline]);

				info = new OpenLayers.Control.WMSGetFeatureInfo({
					layers: [msp_outline,sg_60_2],
							...
							<td valign=top>
								<table border=0>
									<tr>
										...
										<td border=0 valign=top align=right rowspan=18 colspan=3>
											...
											<table border=0 cellpadding=0 cellspacing=0 rules="none">
												<tr>													
													...
													<td width=30>
														<a href="metadaten/metadaten.php?Layer_ID=160200" target="_blank" onclick="return meta_popup(this.href)"><img src="images/info_button.gif" title="Metadaten" border=0></a>
													</td>
													<td align=right>
									...
									<tr>
										<td align="center" height="10" valign=center colspan=2 bgcolor=<? echo $element_farbe ?>>
											<small><a href="bas_zustaendigkeit.php">gesamten Landkreis anzeigen</a></b>
										</td>									
									</tr>
									<tr>
										<td align="center" valign="center" colspan=2 height=40>
											<form action="bas_zustaendigkeit.php" method="get" name="gemeinde">
												Gemeinde:&nbsp;
									...
									for ($i=1;$i<=$ma_count;$i++)
									  {
									   echo "<tr>
										<td colspan=2 align=center height=30 bgcolor=$header_farbe >$font_farbe zuständiger Sachbearbeiter<br>Bauaufsicht $font_farbe_end ;</td>
									   </tr>
	...
	?>
		...
		<script type="text/javascript" language="Javascript">
			...
			function load() {
				...
				map.addLayers([orka_mv,dop,gemeinden_msp,sg_60_2]);

				info = new OpenLayers.Control.WMSGetFeatureInfo({
					layers: [gemeinden_msp,sg_60_2],
					url: '<? echo $featureinfo_msp_url; ?>',
					title: 'Identify features by clicking',
					queryVisible: true,
					eventListeners: {
				...
				<table width="100%" border="0" cellpadding="0" align="center" cellspacing="0">
					<tr>
						<td align="center" valign="top" width=300 height=60 colspan=2>
							<br>
							<h3>Zuständigkeiten<br>Bauaufsicht</h3>
					...
					<tr>
					    ...
					    *) <a href="metadaten/metadaten.php?Layer_ID=<? echo $layerid?>" target="_blank" onclick="return meta_popup(this.href)">Info zum Thema Zuständigkeiten Bauaufsicht</a>
						</td>
					</tr>
					<!--Zeile 4 letzte Aktualisierung -->
					<tr><td align=center colspan=2>letzte Aktualisierung: <b><i><? echo get_i_aktualitaet($db_link,'160200'); ?></i></b></td></tr>
