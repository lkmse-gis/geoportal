...
$query="SELECT b.gemarkungsname_kurz as bezeichnung,a.flurnummer,a.zaehler,a.nenner,a.amtlicheflaeche,c.gemeinde,d.name as amt,a.gemarkungsnummer,a.land ,c.gem_schl ,st_box(st_buffer(a.wkb_geometry,100+'$puffer')) as bounding_box, round(st_area(a.wkb_geometry)::numeric,2) as gerechnet  FROM alkis.ax_flurstueck as a, public.gemarkung as b, public.gemeinden as c, public.fd_amtsbereiche as d WHERE a.flurstueckskennzeichen='$flstkennz' AND a.gemarkungsnummer=b.gemarkung::integer AND c.gem_schl::integer=b.gemeinde::integer AND c.amt_id::integer=d.amts_sf AND a.endet IS NULL";
...
<form name="form">
 <select  style="width:250px;margin-right:25px;" name="link" SIZE="4" onChange="window.location.href = document.form.link.options[document.form.link.selectedIndex].value;">
									 <option value="<? echo"http://geoport-lk-mse.de/geoportal/apps/flstauskunft/flstauskunft_hauptscript_druck_dev.php?flst=$flstkennz&basis=ORKA&puffer=$puffer"; ?>" 
																<?php if($_GET['basis'] == "ORKA") echo "selected=\"selected\"";?>>Offene Regionalkarte (ORKa)</option>
									 <option  value="<? echo"http://geoport-lk-mse.de/geoportal/apps/flstauskunft/flstauskunft_hauptscript_druck_dev.php?flst=$flstkennz&basis=DOP20&puffer=$puffer"; ?>" 
 ...
 <select  style="width:250px;margin-left:25px;" name="link1" SIZE="4" onChange="window.location.href = document.form1.link1.options[document.form1.link1.selectedIndex].value;">
									 <option  value="<? echo"http://geoport-lk-mse.de/geoportal/apps/flstauskunft/flstauskunft_hauptscript_druck_dev.php?flst=$flstkennz&basis=$basis&puffer=-1"; ?>" 
																<?php if($_GET['puffer'] == "-1") echo "selected=\"selected\"";?>>Kein Puffer</option>
									 <option  value="<? echo"http://geoport-lk-mse.de/geoportal/apps/flstauskunft/flstauskunft_hauptscript_druck_dev.php?flst=$flstkennz&basis=$basis&puffer=50"; ?>" 
																<?php if($_GET['puffer'] == "50") echo "selected=\"selected\""; ?>>Puffer 50m</option>
									 <option  value="<? echo"http://geoport-lk-mse.de/geoportal/apps/flstauskunft/flstauskunft_hauptscript_druck_dev.php?flst=$flstkennz&basis=$basis&puffer=100"; ?>" 
																<?php if($_GET['puffer'] == "100") echo "selected=\"selected\""; ?>>Puffer 100m</option>
									 <option  value="<? echo"http://geoport-lk-mse.de/geoportal/apps/flstauskunft/flstauskunft_hauptscript_druck_dev.php?flst=$flstkennz&basis=$basis&puffer=200"; ?>" 
...
echo "<img id=\"flauszug\" alt='Flurst&uuml;cksfehler' src=\"http://www.geoport-lk-mse.de/webservices/alkis07?REQUEST=GetMap&VERSION=1.1.1&SERVICE=WMS&LAYERS=",$basis,",ag_t_flurstueck,ag_l_flurstueck,sk2004_zuordnungspfeil_spitze,ag_p_flurstueck,ax_flurstueck,ax_besondereflurstuecksgrenze,ax_punktortta,ax_bauteil,ax_besonderegebaeudelinie,ag_t_gebaeude,ag_t_nebengeb,ag_l_gebaeude,ax_gebaeude_txt&BBOX=$bbox&SRS=EPSG:25833&FORMAT=image/png&WIDTH=735&HEIGHT=735&STYLES=\">";	
};

################# the_geom Puffer ###########################

$query="SELECT st_buffer(wkb_geometry,'$puffer') as the_geom FROM alkis.ax_flurstueck WHERE flurstueckskennzeichen='$flstkennz' AND endet IS NULL;";
$result = $dbqueryp($connectp,$query);
$r = $fetcharrayp($result);
$the_geom = $r[the_geom];
...
$query="SELECT a.nr,a.lfdnr,a.obj,a.ort
 FROM construction.baudenkmale as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),b.wkb_geometry) AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
$query="SELECT a.bl_bl_nr, a.bl_bl_s, a.ldf_nr, a.art, a.bemerk
 FROM construction.baulasten as a,alkis.ax_flurstueck as b WHERE st_intersects(a.the_geom,st_buffer(b.wkb_geometry,-1)) AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL";
...
FROM environment.sg_lsg_2016 as a,alkis.ax_flurstueck as b WHERE st_intersects(a.geom,st_buffer(b.wkb_geometry,-1)) AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL";
...
while($r = $fetcharrayp($result))
  ...
  {
	$eigentuemer[$counter] = $r[eigentuemer];
	$anschrift[$counter] = $r[kvwmap_anschrift];
	$eigentuemer[$counter] = str_replace("\n", "<br />", str_replace("\r", "", $eigentuemer[$counter]));  
    $counter++;
	};
...
if ($counter>0) 

	{
		echo "
			<table id='customers'>
			<th height='30' colspan='6' >Adresse mit Eigentümer</th>
			<tr id='trinfo'>
...
CASE 
	...
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM construction.baudenkmale  as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM construction.bodendenkmale_blau as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM construction.bodendenkmale_rot as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM construction.baulasten as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM construction.baulasten_msbau as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM fd_vblp5 as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM planning.stoerfallanlagen as a,alkis.ax_flurstueck as b WHERE st_intersects(st_buffer(st_transform(st_buffer(a.the_geom,a.stoerfall_radius),25833),a.stoerfall_radius),st_buffer(b.wkb_geometry,-1)) AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL";
	...
	if ($count> 0) 
		{
			echo "
				<table id='customers' >
				<th  height='30' colspan='6'><b>Störfallbetriebe</th>
				<tr>
...
CASE 
	...
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM protection.kbelastete_gebiete_2015 as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM protection.kberaeumte_gebiete_2015 as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM environment.sg_naturschutzgebiete as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil  FROM environment.sg_nationalparke as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil  FROM environment.sg_spa_fl as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM environment.sg_lsg_2016 as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
CASE 
	...
	THEN (round((st_area(INTERSECTION(st_transform(a.the_geom,25833),'$the_geom'))/st_area('$the_geom'))::numeric*100,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM environment.sg_ffh_fl as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),'$the_geom') AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
