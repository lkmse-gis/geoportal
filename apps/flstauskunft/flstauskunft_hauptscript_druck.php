		...
		<table style="position:absolute; top:35%;left:30%;" >
			<tr>
				<td>
					...
					<div id="loading-text-1"><b>Geoportal<br> Landkreis Mecklenburgische Seenplatte</b><br><br>Flurstücksauskunft <i><small><b>ALKIS<sup>®</sup></b></small></i></div>
...
$query="SELECT b.gemarkungsname_kurz as bezeichnung,a.flurnummer,a.zaehler,a.nenner,a.amtlicheflaeche,c.gemeinde,d.name as amt,a.gemarkungsnummer,a.land ,c.gem_schl ,st_box(st_buffer(a.wkb_geometry,100)) as bounding_box, round(st_area(a.wkb_geometry)::numeric,2) as gerechnet  
FROM alkis.ax_flurstueck as a, public.gemarkung as b, public.gemeinden as c, public.fd_amtsbereiche as d
WHERE a.flurstueckskennzeichen='$flstkennz' AND a.gemarkungsnummer=b.gemarkung AND c.gem_schl::integer=b.gemeinde::integer AND c.amt_id::integer=d.amts_sf AND a.endet IS NULL";
...
//}
//AND st_intersects(a.wkb_geometry,g.wkb_geometry)  
...
$query1= "SELECT a.ortsteil FROM alkis.ax_flurstueck as b,address_registry.adresstabelle as a WHERE b.flurstueckskennzeichen='$flstkennz' AND st_intersects(a.wkb_geometry,b.wkb_geometry) AND b.endet IS NULL";
$result1 = $dbqueryp($connectp,$query1);
$o = $fetcharrayp($result1);		
echo 
	"
	...
	<tr><td id=padding><b>Orts-/Stadtteil:</td><td>$o[ortsteil]<td></td>
...
<form name="form" >
 <select name="link" SIZE="1" onChange="window.location.href = document.form.link.options[document.form.link.selectedIndex].value;">
									 ...
									 <option  value="<? echo"http://geoport-lk-mse.de/geoportal/apps/flstauskunft/flstauskunft_alkis_druck.php?flst=",$flstkennz,"&basis=DOP20"; ?>" 
																<?php if($_GET['basis'] == "DOP20") echo "selected=\"selected\""; ?>>Luftbilder (DOP20)</option>
 </select>
 </form>
</table> 
<br>
...
$query="SELECT a.g_name, a.b_name, a.b_art, a.stoerfall_radius, a.stoerfall_status
FROM planning.stoerfallanlagen as a,alkis.ax_flurstueck as b WHERE st_intersects(st_buffer(st_transform(st_buffer(a.the_geom,a.stoerfall_radius),25833),a.stoerfall_radius),st_buffer(b.wkb_geometry,-1)) AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL AND a.jahrgang='$jahrgang'";
...
FROM protection.kbelastete_gebiete as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.geom,25833),st_buffer(b.wkb_geometry,-1))  AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL";
...
if ($counter>0) 

	{
		echo "
		<table id='customers'>
			<th height='30' colspan='6'><b>Adresse mit Eigentümer</th>
			<tr >
				<td width=100 ><b>Eigentümer</td>
				<td width=100 ><b>Adresse</td>
			</tr>";

		for ($i = 0; $i < $counter; $i++)
		{	
			echo "	<tr >
						<td ><br>$eigentuemer[$i]<br></td>
						<td >$anschrift[$i]</td>
					</tr>";
...
CASE 
	...
	ELSE 'Geometrie fehlerhaft'::text
END as anteil FROM construction.baudenkmale  as a,alkis.ax_flurstueck as b WHERE st_intersects(st_transform(a.the_geom,25833),b.wkb_geometry) AND b.flurstueckskennzeichen='$flstkennz' AND b.endet IS NULL;";
...
while($r = $fetcharrayp($result))
  {
	$stoerfall_jahr[$count] = $r[jahrgang];
...
CASE 
	WHEN st_isvalid(a.geom) = TRUE
	THEN (round(st_area(INTERSECTION(geom,b.wkb_geometry))::numeric,2))::text 
	ELSE 'Geometrie fehlerhaft'::text
END as schnittflaeche, 
CASE 
	WHEN st_isvalid(a.geom) = TRUE
