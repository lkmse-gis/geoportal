...
$jahrgang_status=$_POST["jahrgang_status"];
if ($jahrgang_status=='x') unset($jahrgang_status);
$erledigt=$_POST["erledigt"];
if (!isset($erledigt)) $erledigt='nein';
$jb_id=$_POST["jb_id"];
if ($jb_id=='x') unset($jb_id);
$jahrgang=$_POST["jahrgang"];
if ($jahrgang == 'x') unset($jahrgang);
if (!isset($jahrgang)) $jahrgang='alle';
if (!isset($sb) AND !isset($vstatus) AND !isset($region) AND !isset($vorgangsart) AND !isset($vstatus) AND !isset($art)) $label='Gesamtliste | ';
  else $label="";
if (isset($jb_id))
  {
...
if (isset($jahrgang)) $label=$label."Jahrgang (Eingang):".$jahrgang." | ";
if (isset($jahrgang_status)) $label=$label."Jahrgang (Status):".$jahrgang_status." | ";
if (isset($region)) $label=$label." Region: ".$region." | ";
if (isset($art)) $label=$label." Art: ".$art." | ";
if (isset($vstatus)) $label=$label." Status (gruppiert): ".$vstatus." | ";
if (isset($e_status)) $label=$label." Status: ".$e_status." | ";
if (isset($vorgangsart)) $label=$label." Vorgangsart: ".$vorgangsart." | ";
...
$query="SET search_path = jagdkataster; SELECT DISTINCT a.oid,a.gid, a.az,a.eingang,a.sb,a.art,a.vorgangsart,a.status,a.datum_status, a.status_gruppe,a.person, a.jagdbezirke,a.gebuehr,a.huel FROM vorgangsliste as a LEFT JOIN vorgangsverwaltung2jb as b ON (a.az=b.az) WHERE ";
        ...
        if ($jahrgang == 'alle') $query=$query. "(1=1) ";
		  else $query=$query."(date_part('year',a.eingang) = '$jahrgang') ";
		...
		if (isset($jahrgang_status)) $query=$query." AND jahrgang_status LIKE '$jahrgang_status' ";
		if (isset($e_status)) $query=$query." AND status LIKE '$e_status' ";
		if (isset($jb_id)) $query=$query." AND b.new_gid='$jb_id' ";
		if ($erledigt == 'nein') $query=$query."  AND a.erledigt IS NULL ORDER BY a.eingang;";
		  else $query=$query."	AND a.erledigt IS NOT NULL ORDER BY a.eingang;";
		#echo $query;
	  $result = $dbqueryp($connectp,$query);
	  $z=0;
...
$query="SELECT DISTINCT b.name,b.new_gid FROM jagdkataster.vorgangsverwaltung2jb as a,jagdkataster.jagdbezirke as b,jagdkataster.vorgangsliste as c WHERE a.new_gid=b.new_gid AND a.az=c.az ";	
        if ($jahrgang=='alle') $query=$query." AND (1=1) ";
           else $query=$query." AND (date_part('year',c.eingang) = '$jahrgang') ";		
		...
		if (isset($e_status)) $query=$query." AND c.status LIKE '$e_status' ";
		if (isset($jahrgang_status)) $query=$query." AND c.jahrgang_status LIKE '$jahrgang_status' ";
		if ($erledigt == 'nein') $query=$query."  AND c.erledigt IS NULL ORDER BY name;";
		  else $query=$query."	AND c.erledigt IS NOT NULL ORDER BY name;";
...
<? 
      echo "<table border=0><tr><td colspan=12 align=left";
	  if ($erledigt == 'ja') echo ' bgcolor=#fee6ce><b> archivierte Vorgänge -> ';
	     else echo ' bgcolor=#9ecee1><b> laufende Vorgänge -> ';
	  echo $label," - ";
...
<a href="javascript:window.print()"><img src="../../kvwmap/graphics/button_drucken.gif" width=20 title="Seite ausdrucken. Bitte Querformat einstellen!" border=0></a>
  </td><td>Jahrgang<br>Eingang</td>
  <td width=30>Region:</td><td>Art:</td><td>Vorgangsart:</td><td>SB:</td><td>Jagdbezirk:</td><td>Status<br>gruppiert</td><td>Jahrgang<br>Status</td><td>Status:</td><td></td><td width=100>HUEL vorhanden:</td></tr>
  <tr><td>';
  #echo '<form action="',$_SERVER["PHP_SELF"],'" method=POST name="gesamtliste">
  #<input type=submit value="Gesamt">
  #</form>';
  
  echo '<form action="',$_SERVER["PHP_SELF"],'" method=POST name="erledigt">';
  ...
  if ($erledigt == 'ja') echo '<input type="hidden" name="erledigt" value="nein">';
     else echo '<input type="hidden" name="erledigt" value="ja">';
  if ($erledigt == 'ja') echo '<input type=submit value="zur Vorgangsliste">';
     else echo '<input type=submit value="zum Archiv">';
  echo '</form>';
  ...
  if (isset($sb)) echo '<input type="hidden" name="sb" value="',$sb,'">';
  if (isset($erledigt)) echo '<input type="hidden" name="erledigt" value="',$erledigt,'">';
  if (isset($region)) echo '<input type="hidden" name="region" value="',$region,'">';
  if (isset($huel)) echo '<input type="hidden" name="huel" value="',$huel,'">';
  if (isset($vorgangsart)) echo '<input type="hidden" name="vorgangsart" value="',$vorgangsart,'">';
  if (isset($art)) echo '<input type="hidden" name="art" value="',$art,'">';
  ...
  echo '<select  name="jahrgang" onchange="document.jahrgang_form.submit();">
			...
			$query="SELECT DISTINCT date_part('year',eingang) as jahrgang_eingang FROM jagdkataster.vorgangsliste  ORDER BY date_part('year',eingang)";
	        $result = $dbqueryp($connectp,$query);
             while($r = $fetcharrayp($result))
		      {
  		       echo "<option ";
		       echo" value=\"$r[jahrgang_eingang]\"";
		       if ($r[jahrgang_eingang] == $jahrgang) echo "selected";
		       echo ">";
		       echo "$r[jahrgang_eingang]</option>\n";
		      }
		    echo "</select></form></td>";
  ...
  if (!isset($e_status))
   {  
   echo '<td>
  <form action="',$_SERVER["PHP_SELF"],'" method=POST name="status_gruppe_form">
  <input type="hidden" name="jahrgang" value=',$jahrgang,'>';
  if (isset($region)) echo '<input type="hidden" name="region" value="',$region,'">';
  if (isset($art)) echo '<input type="hidden" name="art" value="',$art,'">';
  if (isset($huel)) echo '<input type="hidden" name="huel" value="',$huel,'">';
  ...
  if (isset($e_status)) echo '<input type="hidden" name="e_status" value="',$e_status,'">';
  if (isset($jahrgang_status)) echo '<input type="hidden" name="jahrgang_status" value="',$jahrgang_status,'">';
  echo '<select  name="vstatus" onchange="document.status_gruppe_form.submit();">
			<option value="x" >alle</option>';
    ...
    echo '<td>
  <form action="',$_SERVER["PHP_SELF"],'" method=POST name="jahrgang_status_form">
  <input type="hidden" name="jahrgang" value=',$jahrgang,'>';
  if (isset($region)) echo '<input type="hidden" name="region" value="',$region,'">';
  if (isset($art)) echo '<input type="hidden" name="art" value="',$art,'">';
  if (isset($huel)) echo '<input type="hidden" name="huel" value="',$huel,'">';
  ...
  if (isset($vstatus)) echo '<input type="hidden" name="vstatus" value="',$vstatus,'">';
  if (isset($jb_id)) echo '<input type="hidden" name="jb_id" value="',$jb_id,'">';
  echo '<select  name="jahrgang_status" onchange="document.jahrgang_status_form.submit();">
			<option value="x" >alle</option>';
			$query="SELECT DISTINCT jahrgang_status FROM jagdkataster.vorgangsliste WHERE jahrgang_status != '' ORDER BY jahrgang_status";
	        $result = $dbqueryp($connectp,$query);
             while($r = $fetcharrayp($result))
		      {
  		       echo "<option ";
		       echo" value=\"$r[jahrgang_status]\"";
		       if ($r[jahrgang_status] == $jahrgang_status) echo "selected";
		       echo ">";
		       echo "$r[jahrgang_status]</option>\n";
		      }
		    echo "</select></form></td>";
  ...
  if (isset($vstatus)) echo '<input type="hidden" name="vstatus" value="',$vstatus,'">';
  if (isset($jahrgang_status)) echo '<input type="hidden" name="jahrgang_status" value="',$jahrgang_status,'">';
  echo '<select  name="e_status" onchange="document.status_form.submit();">
			<option value="x" >alle</option>';
			if (!isset($vstatus)) $query="SELECT status FROM jagdkataster.vorgang_status ORDER BY gruppe";
			  else $query="SELECT status FROM jagdkataster.vorgang_status WHERE gruppe = '$vstatus' ORDER BY gruppe";
	        $result = $dbqueryp($connectp,$query);
             while($r = $fetcharrayp($result))
		      {
  		       echo "<option ";
		       echo" value=\"$r[status]\"";
		       if ($r[status] == $e_status) echo "selected";
		       echo ">";
...
echo "<td height=30 width=80><b>Eingang</b></td>
 ...
 for($v=0;$v<$z;$v++)
	{   
			...
			if ($erledigt == 'ja') echo "<tr bgcolor='#fee6ce'";
		...
		if ($status == 'angemeldet') 
		  
		     {
			   echo '<td align=center>
			    <a href="https://geoport-lk-mse.de/kvwmap/index.php?go=Layer-Suche_Suchen&operator_az==&selected_layer_id=60076&value_az=',$bd[$v][az],'"><img src="../../kvwmap/graphics/button_ansicht.png" width=20 title="Objekt bearbeiten" border=0></a>';
				if ($erledigt == 'nein') {
				echo '<a href="https://geoport-lk-mse.de/geoportal/apps/jagdkataster/liste_vorgang_del.php?delete_id=',$bd[$v][gid];
				...
				if (isset($vstatus)) echo '&vstatus=',$vstatus;
				if (isset($e_status)) echo '&vstatus=',$e_status;
				if (isset($jahrgang)) echo '&jahrgang=',$jahrgang;
				if (isset($jahrgang_status)) echo '&jahrgang_status=',$jahrgang_status;
				echo '"><img src="../../kvwmap/graphics/haken.png" width=10 title="Vorgang archivieren" onClick="return sicher()" border=0></a>';
				}
				echo '</td>';
			  }
			...
			if ($bd[$v][status_gruppe] == '3 erledigt' OR $bd[$v][status_gruppe] == '4 Widerspruch') echo '<td align=right>',$bd[$v][gebuehr],'</td>';
			 else echo '<td></td>';
