...
if ($gemeinde_id > 0)
   { 	  
	  $query="SELECT a.*,b.gemeinde as gemeindename,b.amt as amtname,c.plz as postleitzahl FROM $schema.$tabelle as a, gemeinden as b,fd_plz as c WHERE ST_WITHIN(a.the_geom, b.the_geom) AND  ST_WITHIN(a.the_geom, c.the_geom) AND b.gem_schl='$gemeinde_id' ORDER BY a.ort";
	  $result = $dbqueryp($connectp,$query);
	  $z=0;
	  while($r = $fetcharrayp($result))
	    {
							...
							<td valign=top>
								<table border=0>
									...
									<tr bgcolor=<? echo $header_farbe; ?>>
										...
										<td align=center>
											<a href="<? echo $datei;?>"><? echo $font_farbe ;?>alle <? echo $titel2 ;?><? echo $font_farbe_end ;?></a>
										</td>
...
<?  }
 ...
 if ($themen_id > 0)
   {  
// für die Überschrift 
	  $query="SELECT a.amt, a.amt_id, a.gemeinde as gemeindename, a.gem_schl as gemeindeid, b.*,c.plz as postleitzahl FROM gemeinden as a, $schema.$tabelle as b,fd_plz as c WHERE ST_WITHIN(b.the_geom, a.the_geom) AND ST_WITHIN(b.the_geom, c.the_geom) AND b.id='$themen_id'";
	  $result = $dbqueryp($connectp,$query);
	  $r = $fetcharrayp($result);
...
// für das Koordinatenfenster	  
	  $query="SELECT astext(the_geom) as koordinaten, astext(st_transform(the_geom, 25833)) as utm, astext(st_transform(the_geom, 4326)) as geo,astext(st_transform(the_geom, 31468)) as rd83 FROM $schema.$tabelle WHERE id='$themen_id'";
	  $result = $dbqueryp($connectp,$query);
	  $xyr = $fetcharrayp($result);
	  
	  
	  $koord = $xyr[koordinaten];
							...
							<td valign=top>
								<table border=0>
									...
									<tr bgcolor=<? echo $header_farbe; ?>>
										...
										<td align=center colspan=2>
											<a href="<? echo $datei;?>"><? echo $font_farbe ;?>zu allen <? echo $titel2 ;?><br><? echo $font_farbe_end ;?></a>
										</td>
								...
								<td valign=top>											
									<table border=0 valign=top>
										<tr height="35">
											<td colspan=3 width="620" bgcolor=<? echo $header_farbe ;?>>&nbsp;&nbsp;<? echo $font_farbe ;?><font size="+1">Wertstoffhof <? echo $gemeindename ;?><? echo $font_farbe_end ;?></td>													
										</tr>
										<tr>
