										...
										<!-- Tabelle für Legende -->											
											<table border="1" rules="none" width=140 valign=bottom align=right>					
												...
												<tr>
													...
													<td align=right><img src="images/spa_pkt.gif" width=20></td>
												</tr>
...
if ($gemarkung_id > 0)
   { 	  
	  $query="SELECT a.* FROM tourism.open_gardens as a, public.gemarkung as b WHERE ST_WITHIN (a.the_geom, b.the_geom) AND a.aktiv = 'ja' AND CAST(b.geographicidentifier AS INTEGER)='$gemarkung_id';";
	  $result = $dbqueryp($connectp,$query);
	  $z=0;
	  while($r = $fetcharrayp($result))
	    {
						...
						<tr>
							<td valign=top>
								...
								<select name="gemarkung" onchange="document.gemarkung.submit();" style="font-family:Arial; font-size: 8pt; font-weight: bold">
								    <option>Bitte auswählen</option>
									<?php
									 $query="SELECT DISTINCT a.gemarkung,a.gemkgschl FROM public.show_gemarkungen as a, public.gemarkung as b, tourism.open_gardens as c WHERE ST_WITHIN(c.the_geom,b.the_geom) AND c.aktiv='ja' AND CAST(b.geographicidentifier as INTEGER)=a.gemkgschl ORDER BY gemarkung";
									 $result = $dbqueryp($connectp,$query);
								...
								<table border=0 width="100%" valign=top>
									...
									<tr>
										<? if ($count>0) echo "
											...
											<td height=30 width=200>&nbsp;<b>Flyer-Nummer:</b></td>
											<td height=30 width=150>&nbsp;<b>Telefon:</b></td>
											";
										?>							
									</tr>																
									<?php for($v=0;$v<$z;$v++)
										{ 
											...
											echo "<td>&nbsp;<a href=\"gardens_msp.php?garten=",$garten[$v][gid],"\">",$garten[$v][name],"<br>",$garten[$v][name2],"</a></td>";														
											echo "<td>&nbsp;",$garten[$v][plz_gaertner]," ",$garten[$v][ort_gaertner],"<br>&nbsp;",$garten[$v][strasse],"</td>
											<td>&nbsp;",$garten[$v][beschriftung],"</td><td>&nbsp;",$garten[$v][tel],"</td>								
											</tr>";
										}
									?>																																				
								</table>
...
if ($garten_id > 0)
   {   
	  $query="SELECT a.gemarkungsname_kurz,a.geographicidentifier FROM public.gemarkung as a, tourism.open_gardens as b WHERE ST_WITHIN(b.the_geom,a.the_geom) AND b.gid='$garten_id';";
	  $result = $dbqueryp($connectp,$query);
	  $r = $fetcharrayp($result);
	  $gemarkungsname=$r[gemarkungsname_kurz];
	  $gemkg_schl=$r[geographicidentifier];
	  
	  $query="SELECT astext(the_geom) as koordinaten, astext(st_transform(the_geom, 25833)) as utm,  st_astext(st_centroid(st_transform(the_geom, 31468))) as rd83, astext(st_transform(the_geom, 4326)) as geo, gid, name_gaertner,strasse_gaertner,plz_gaertner,ort_gaertner,oeffnungszeiten,beschreibung FROM tourism.open_gardens WHERE gid='$garten_id'";
					...
					<table width="100%" border="0" cellpadding="0" align="center" cellspacing="0">
						...
						<tr>
							<td valign=top>											
								...
								?>
									<tr height="35">
										<td colspan=<? echo $spaltenanzahl ?> width="620" bgcolor=<? echo $header_farbe ;?>>&nbsp;&nbsp;<font size="+1"><? echo $font_farbe ;?><? echo $r[beschriftung]," ",$r[name];?><? echo $font_farbe_end ;?></td>													
									</tr>
									<tr>
										<td bgcolor=<? echo $element_farbe ?>>Postleitzahl/Ort:</td>
										<td bgcolor=<? echo $element_farbe ?>><b><? echo $r[plz_gaertner_gaertner]," ",$r[ort_gaertner_gaertner] ;?></b></td>
									</tr>
									<tr>
										<td>Straße:</td>
