...
<?
			  ...
			  else if ($hoch_range > 1000 AND $hoch_range < 3999 OR $rechts_range > 1000 AND $rechts_range < 3999) $zoom=26;
			  else if ($hoch_range > 500 AND $hoch_range < 999 OR $rechts_range > 500 AND $rechts_range < 999) $zoom=27;
              else $zoom=28;
?>
<?
	echo"
			...
			function load() {
				...
				var ortsteile = new OpenLayers.Layer.WMS.Untiled(\"Ortsteile\",
								 $map_msp_url,
								 {layers: 'ortsteile_lt_rka', transparent: true, format: 'image/png'},
								 {isBaseLayer: false}
				);
				
				var strassenzuege = new OpenLayers.Layer.WMS.Untiled(\"Strassenzug\",
								 $map_msp_url,
								 {layers: 'strassenzuege', transparent: true, format: 'image/png'},
								 {isBaseLayer: false}
				);
				...
				map.addLayers([orka_mv,webatlasde,dtkmv,dop,ortsteile,strassenzuege,thema]);
