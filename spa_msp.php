...
//globale Varibalen
$titel="Vogelschutzgebiete";
$titel2="Vogelschutzgebiet";
$datei="spa_msp.php";
$tabelle="sg_spa_fl";
$schema="environment";
$kuerzel="spafl";
								...
								<tr>
									<td align="center" height=60 colspan=2>
										...
										<select name="<? echo $kuerzel;?>" onchange="document.<? echo $kuerzel;?>.submit();" style="width: 200px;">
											<?php
												$query="SELECT gebiet_nam, gid FROM $schema.$tabelle ORDER BY gebiet_nam";
												$result = $dbqueryp($connectp,$query);
												echo "<option>Bitte ausw&auml;hlen</option>\n";
												while($r = $fetcharrayp($result))
													{
														echo "<option value=\"$r[gid]\">$r[gebiet_nam]</option>\n";
													}
											?>
										</select>										
										</form>
										...
										<td valign=bottom align=right>
											...
											<table border="1" rules="none" width=140 valign=bottom align=right>					
												...
												<tr>
													...
													<td align=right><img src="images/spafl.gif" width=30></td>													
...
if ($spafl_id > 0)
   {   
	  $query="SELECT a.name, a.amts_sf FROM fd_amtsbereiche as a, $schema.$tabelle as b WHERE ST_INTERSECTS(b.the_geom,st_transform(a.the_geom,5650)) AND b.gid='$spafl_id' ORDER by a.name";
	  $result = $dbqueryp($connectp,$query);
	  $i=0;
	  while($r = $fetcharrayp($result))
	    {
	  ...
	  $query="SELECT a.gem_schl, a.gemeinde FROM gemeinden as a, $schema.$tabelle as b WHERE ST_INTERSECTS(b.the_geom,st_transform(a.the_geom,5650)) AND b.gid='$spafl_id' ORDER by a.gemeinde";
	  $result = $dbqueryp($connectp,$query);
	  $k=0;
	  while($r = $fetcharrayp($result))
	    {
	  ...
	  $query="SELECT box(the_geom) as box, area(the_geom) as area, st_astext(st_centroid(st_transform(the_geom,2398))) as center, st_astext(st_centroid(st_transform(the_geom, 25833))) as utm, st_astext(st_centroid(st_transform(the_geom, 31468))) as rd83, st_astext(st_centroid(st_transform(the_geom, 4326))) as geo, st_astext(st_transform(the_geom, 2398)) as koordinaten, st_perimeter(the_geom) as umfang, gid, gebiet_nam, eu_nr, nr, gis_code FROM $schema.$tabelle WHERE gid='$spafl_id'";	  
	  $result = $dbqueryp($connectp,$query);
	  $r = $fetcharrayp($result);
	  $bildname = $r[ziel];
	  $area=$r[area];	  
								...
								<table border=0>									
									<tr>
										<td height="40" align="center" valign=center width=250 colspan="2" bgcolor=<? echo $header_farbe; ?>>
											<? echo $font_farbe ;?><? echo $r[gebiet_nam]; ?><? echo $font_farbe_End ;?>
										</td>
											...
											<form action="<? echo $datei;?>" method="get" name="<? echo $kuerzel;?>">
												<select name="<? echo $kuerzel;?>" onchange="document.<? echo $kuerzel;?>.submit();" style="width: 200px;">
													<?php
														...
														{
														 echo "<option";if ($spafl_id == $e[gid]) echo " selected"; echo " value=\"$e[gid]\">$e[gebiet_nam]</option>\n";
														}
													?>
												</select>
											</form>
									...
									<tr>										
										<td valign=bottom align=right>
											...
											<table border="1" rules="none" width=140 valign=bottom align=right>					
												...
												<tr>
													<td align=right><small>Vogelschutzgebiet: </td>
													<td align=right><img src="images/spafl.gif" width=30></td>
													<td align=right><small>Kreisgrenze: </td>
													<td align=right><img src="images/gemeindegrenze_2.png" width=30></td>
												</tr>																							
									...
									<tr>
										...
										<td align=right>
											<a href="<? echo $datei;?>?<? echo $kuerzel;?>=<? echo $nsg_id; ?>"><img src="images/reload.png" title="Kartenausschnitt neu laden"></a>
										</td>
									</tr>
								</table>
							</td>
					...
					<table width="100%" border="0" cellpadding="0" align="center" cellspacing="0">
							...
							<td valign=top>											
								<table border=0 valign=top>
									<tr height="35">
										<td colspan=3 width="620" bgcolor=<? echo $header_farbe ;?>>&nbsp;&nbsp;<? echo $font_farbe ;?><font size="+1"><? echo $r[gebiet_nam] ;?><? echo $font_farbe_end ;?></td>													
									</tr>
									<tr>
										<td bgcolor=<? echo $element_farbe ?>>Nummer:</td>
										<td bgcolor=<? echo $element_farbe ?>><b><? echo $r[nr] ;?></b></td>													
									</tr>
									<tr>
										<td>EU-Nummer:</td>
									...
									<tr height="30">
										<td bgcolor=<? echo $element_farbe ?>>Anlage:</td>
										<?
											echo "<td bgcolor=$element_farbe><b><a href='pictures/spa_fl/$r[gis_code].pdf' target='_blank' onclick='return popup(this.href);'>$r[gis_code].pdf</a></b></td>";
										?>
									</tr>
									<tr height="30">
										<td>GIS Code: </td>
										<td><b><? echo $r[gis_code] ;?></b></td>
									<tr>
										<td bgcolor=<? echo $element_farbe ?> valign=top>Vogelschutzgebiet schneidet folgende<br>&Auml;mter des Kreises:</td>
									...
									<tr>
										<td valign=top>Vogelschutzgebiet schneidet folgende<br>Gemeinden des Kreises:</td>
										<td><b>
											<?php 
												for($y=0;$y<$k;$y++)
